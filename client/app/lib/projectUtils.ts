...
import { BASE_PATH, API_BASE } from './util';
...
export async function readMetaFile(projectName: string): Promise<ProjectEntry | null> {
  ...
  const statsRes = await fetch(`${API_BASE}/history-files/${encodeURIComponent(folderName)}`);
  ...
  const metaRes = await fetch(`${API_BASE}/meta?file=${encodeURIComponent(metaFile)}`);
  let metaRecords: any[] = [];
  if (metaRes.ok) {
...
export async function readMetaFileExcel(projectName: string): Promise<ProjectEntry | null> {
  ...
  const res = await fetch(`${API_BASE}/meta?file=${encodeURIComponent(file)}`);
