...
* Results are written under results/sonnet_runs_VAERS by default.
...
/compute001/lwu/projects/LLM4AE/LLM4AE-dev/publication/
    ...
    results/sonnet_runs_VAERS/

Examples
--------
python scripts/run_VAERS_sonnet.py
python scripts/run_VAERS_sonnet.py --limit 10
python scripts/run_VAERS_sonnet.py --no-resume
python scripts/run_VAERS_sonnet.py --timeout 600 --max-output-tokens 32768
...
DEFAULT_RESULTS_DIR = BASE / "results" / "sonnet_runs_VAERS"
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("llm4ae_sonnet_vaers")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "One-pass Claude Sonnet VAERS annotation using a VAERS P2 tagged-text prompt "
...
def main() -> None:
    ...
    # Load Elsa configuration from .env. Credentials are never printed or persisted.
    ...
    console_print("")
    console_print("=== CLAUDE SONNET VAERS EVALUATION ===")
    console_print(
