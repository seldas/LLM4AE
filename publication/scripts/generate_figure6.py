...
def plot_figure(bert: Evaluation, llama: Evaluation, output_path: Path, seed: int) -> None:
    ...
    bert_counts_b = [bert.counts[code] for code in codes]
    llama_counts_b = [llama.counts[code] for code in codes]
    ...
    error_width = 0.35
    ax_b.bar(
        x_error - error_width / 2, bert_counts_b, error_width, color=bert_color,
        edgecolor="#111111", linewidth=0.6, label="BERT", zorder=2,
    )
    ax_b.bar(
        x_error + error_width / 2, llama_counts_b, error_width, color=llama_color,
        edgecolor="#111111", linewidth=0.6, label="LLaMA-4", zorder=2,
    )
    count_y_max = max(bert_counts_b + llama_counts_b)
    for index, code in enumerate(codes):
        for offset, count, pct, color in (
            (-error_width / 2, bert_counts_b[index], bert_pct[index], bert_dark),
            (error_width / 2, llama_counts_b[index], llama_pct[index], llama_dark),
        ):
            ax_b.text(
                x_error[index] + offset, count + count_y_max * 0.01,
                f"{count:,}\n({pct:.1f}%)",
                ha="center", va="bottom", fontsize=8,
                fontweight="bold", color=color,
            )
    ax_b.set_title(
        "(b) VAERS M/C/S/N Outcome Distribution",
        fontsize=11.5, fontweight="bold", loc="left",
    )
    ax_b.set_xticks(x_error)
    ax_b.set_xticklabels(error_labels, fontsize=9, fontweight="bold")
    ax_b.set_ylim(0, count_y_max * 1.25)
