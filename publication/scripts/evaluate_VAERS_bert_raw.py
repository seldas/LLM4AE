#!/usr/bin/env python3
"""
evaluate_VAERS_bert_raw.py

Evaluate trained VAERS BioBERT models directly on the FULL raw narrative texts
of held-out test cases, rather than the processed/filtered test.spacy files.

Background & Motivation:
-----------------------
In the previous 10-fold CV pipeline (run_VAERS_bert.py), test.spacy was generated via
...
This script:
  1. Identifies the exact test cases for each of the 10 folds from the experiment
     manifest (publication/results/bert_runs_VAERS/experiment_manifest.json) or
     deterministic 10-fold partition (cv_seed=42).
...
Usage Examples:
---------------
  # Evaluate all models in a work directory across 10 folds x 5 seeds:
  python publication/scripts/evaluate_VAERS_bert_raw.py \
      --work-dir publication/results/bert_runs_VAERS/workdir \
      --output-dir publication/results/bert_retest_VAERS_raw

  # Evaluate a single model checkpoint on fold 0, seed 42:
  python publication/scripts/evaluate_VAERS_bert_raw.py \
      --model-path /path/to/fold_00/seed_42/model/model-best \
      --fold 0 --seed 42

  # Dry-run validation (verifies data loading, manifest tracking, and fold splits without model):
  python publication/scripts/evaluate_VAERS_bert_raw.py --dry-run
"""
...
DEFAULT_MANIFEST = PROJECT_ROOT / "results" / "bert_runs_VAERS" / "experiment_manifest.json"
DEFAULT_REF_SCORER = PROJECT_ROOT / "code" / "custom_scorer_v5.py"
DEFAULT_RESULTS_DIR = PROJECT_ROOT / "results" / "bert_retest_VAERS_raw"
...
# -----------------------------------------------------------------------------
# Data Loading & Manifest Tracking
# -----------------------------------------------------------------------------
def load_vaers_raw_records(db_path: Path) -> Tuple[Dict[str, dict], Dict[str, Any]]:
    """
    Load all VAERS whole cases and their SME1 annotations from dataset.db.

    Returns:
        records_by_id: dict mapping doc_id (e.g. 'VAERS_133422-1') to record dict:
    ...
    docs_rows = cursor.execute(
        """
        SELECT doc_id, base_id, suffix, page_text
        FROM documents
        WHERE dataset = 'VAERS'
        ORDER BY doc_id
        """
    ...
    anns_rows = cursor.execute(
        ...
        FROM annotations
        JOIN documents ON documents.doc_id = annotations.doc_id
        WHERE note = 'SME1' AND documents.dataset = 'VAERS'
    ...
    stats["documents_loaded"] = len(records_by_id)
    return records_by_id, dict(stats)


def get_vaers_fold_partition(
    ...
    """
    Get document IDs assigned to each of the 10 folds.
    Uses experiment_manifest.json if available, else deterministically recreates
    the identical partition using make_cv_folds(cv_seed=42).
    """
    if manifest_path and manifest_path.is_file():
        try:
            with manifest_path.open(encoding="utf-8") as f:
                manifest_data = json.load(f)
            fold_docs = manifest_data.get("fold_documents", {})
            partition = {}
            for k_str, filenames in fold_docs.items():
                k = int(k_str)
                # Map 'VAERS_550858-1.json' -> 'VAERS_550858-1'
                doc_ids = [Path(fn).stem for fn in filenames]
                partition[k] = doc_ids
            return partition
        except Exception as e:
            print(f"[WARN] Failed to read manifest ({e}), falling back to deterministic split.")

    # Deterministic fallback matching run_VAERS_bert.py
    sorted_doc_ids = sorted(records_by_id.keys())
    shuffled = list(sorted_doc_ids)
    random.Random(cv_seed).shuffle(shuffled)
    partition = {i: [] for i in range(n_folds)}
    for i, doc_id in enumerate(shuffled):
        partition[i % n_folds].append(doc_id)
    return partition
...
def evaluate_raw_test_records(
    ...
    all_rows = []
    iterator = tqdm(test_recs, desc=f"Eval Fold {fold_idx:02d} (seed {seed})", unit="doc") if show_progress else test_recs
...
# -----------------------------------------------------------------------------
def discover_model_runs(work_dir: Path) -> List[Tuple[int, int, Path]]:
    """
    Locate saved model checkpoints in a work directory.
    Matches:
      - fold_<idx>/seed_<seed>/model/model-best
    ...
    for mb_path in sorted(work_dir.glob("**/model-best")):
        ...
        for part in parent_parts:
            ...
            if m_fold:
                fold_idx = int(m_fold.group(1))
                break

        if fold_idx is not None:
            discovered.append((fold_idx, seed, mb_path))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Re-evaluate trained VAERS BioBERT models on raw whole-case narratives."
    )
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=PROJECT_ROOT / "results" / "bert_runs_VAERS" / "workdir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Specific fold index (0 to 9).",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Path to experiment_manifest.json (default: {DEFAULT_MANIFEST}).",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate raw data tracking and fold partition without model inference.",
    )
...
def main():
    ...
    print("=" * 78)
    print("VAERS Raw Narrative BioBERT Evaluation Suite (10-Fold CV)")
    print("=" * 78)
    ...
    records_by_id, stats = load_vaers_raw_records(args.db_path)
    print(f"\n[DATA] Loaded {stats['documents_loaded']} VAERS documents with {stats['annotations_kept']} SME1 annotations.")

    # 2. Get 10-fold partition
    partition = get_vaers_fold_partition(args.manifest, records_by_id)
    print("\n10-Fold Partition:")
    for fold_k in sorted(partition.keys()):
        doc_ids = partition[fold_k]
        fold_anns = sum(len(records_by_id[did]["annotations"]) for did in doc_ids if did in records_by_id)
        print(f"  - Fold {fold_k:02d}: {len(doc_ids):3d} documents, {fold_anns:5d} gold annotations")

    if args.dry_run:
        print("\n[DRY-RUN] Data validation successful. All 10 folds tracked to raw narratives.")
    ...
    # 4. Identify models to run
    runs_to_eval = []
    if args.model_path is not None:
        fold_idx = args.fold if args.fold is not None else 0
        runs_to_eval.append((fold_idx, args.seed, args.model_path))
    else:
    ...
    for f_idx, s, p in runs_to_eval:
        print(f"  - Fold: {f_idx:02d} | Seed: {s:4d} | Path: {p}")
    ...
    # 5. Run evaluation
    for fold_idx, seed, model_path in runs_to_eval:
        test_doc_ids = partition.get(fold_idx, [])
        test_recs = [records_by_id[did] for did in test_doc_ids if did in records_by_id]
        print(f"\nEvaluating Fold {fold_idx:02d} (seed {seed}) on {len(test_recs)} raw test cases...")
        ...
        overall_row, cat_df = summarize_fold_eval(raw_df, fold_idx, seed)
    ...
    with pd.ExcelWriter(summary_path, engine="openpyxl") as writer:
        ...
        cat_summary.to_excel(writer, sheet_name="By_Category", index=False)

        if "fold" in overall_summary.columns:
            agg = overall_summary.groupby("fold")[["strict_F1", "ade_F1"]].agg(["mean", "std"])
