...
def show_project(project_name):
    try:
        ...
        query = '''
            ...
            LIMIT ? OFFSET ?
        '''
        count_query = 'SELECT COUNT(*) as total_count FROM cases'

        rows = conn.execute(query, (limit, offset)).fetchall()
        total_count = conn.execute(count_query).fetchone()['total_count']
        conn.close()
...
@cross_origin()
def delete_project():
    return jsonify({'message': 'Project deletion disabled in API mode'}), 200
