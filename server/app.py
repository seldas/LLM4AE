...
@app.route("/api/annotate_icsr_intake/", methods=["POST", "HEAD"])
@app.route("/api/annotate_icsr_intake", methods=["POST", "HEAD"])
@cross_origin()
def annotate_icsr_intake():
    ...
    try:
        case_data_str = request.form.get("case_data")
        if not case_data_str:
            req_json = request.get_json(silent=True) or {}
            case_data_str = req_json.get("case_data")
            
        if not case_data_str:
            return jsonify({"error": "Missing case_data"}), 400
            
        case_data = json.loads(case_data_str)
        external_case_id = str(case_data.get("id"))
        version = str(case_data.get("version", "1"))
        safety_report_id = str(case_data.get("safety_report_id", external_case_id))
        narrative = case_data.get("narrative", "")
        
        raw_annotations = case_data.get("annotations") or {}
        items = raw_annotations.get("items") or []
        
        filename = f"{safety_report_id}-{version}.json"
        
        attrs = {
            "case_number": safety_report_id,
            "version_number": version,
            "narrative": narrative,
            "pages": json.dumps([narrative]),
            "annotate_filename": filename,
            "meta": json.dumps(raw_annotations.get("meta") or {})
        }
        
        case_id = upsert_case(safety_report_id, version, attrs)
        ...
        try:
            ...
            if isinstance(items, list):
                for item in items:
                    ...
                    text = item.get("text_content") or item.get("textContext", {}).get("text") or ""
                    start = item.get("start_offset") if item.get("start_offset") is not None else item.get("textContext", {}).get("start", 0)
                    end = item.get("end_offset") if item.get("end_offset") is not None else item.get("textContext", {}).get("end", 0)
                    role_name = item.get("role_name") or item.get("note") or "Admin"
                    
                    user_id = get_user_by_note(role_name) or 1
                    relationships = json.dumps(item.get("relationships") or {})
        ...
        redirect_url = f"/annotate?project=askMyFAERS&id={case_id}"
        ...
        <head>
          <meta charset="utf-8">
          <title>Redirecting to SCAT...</title>
          <script>
            localStorage.setItem('user', JSON.stringify({{"id": 1, "username": "admin", "full_name": "System Administrator", "migration_key": null}}));
            window.location.href = "{redirect_url}";
          </script>
        </head>
        <body>
          <p style="font-family: sans-serif; text-align: center; margin-top: 50px; color: #4b5563;">
            Initialising SCAT annotation session...
          </p>
        </body>
        </html>
        """
        ...
        logging.error(f"Error in annotate_icsr_intake: {e}")
        return jsonify({"error": str(e)}), 500
