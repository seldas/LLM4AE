...
def get_case(case_id=None, project_id=None, filename=None):
    ...
    try:
        ...
        res = conn.execute('SELECT * FROM cases WHERE annotate_filename = ?', (filename,)).fetchone()
        
        if not res and filename and filename.lower().endswith('.json'):
            # Try without .json suffix
            no_json = filename[:-5]
            res = conn.execute('SELECT * FROM cases WHERE annotate_filename = ?', (no_json,)).fetchone()
            
        if not res and filename and not filename.lower().endswith('.json'):
            # Try with .json suffix
            with_json = filename + ".json"
            res = conn.execute('SELECT * FROM cases WHERE annotate_filename = ?', (with_json,)).fetchone()
