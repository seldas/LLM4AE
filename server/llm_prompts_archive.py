...
## Archived Prompts
## June 2025
prompt_ner_old= '''**You are tasked with annotating key clinical and contextual attributes in a FAERS case report narrative. Follow the instructions below carefully.**

### ✳️ Annotation Format
- Perform **in-text annotation** by surrounding the entity of interest with appropriate XML-style tags.
- **Do not change the original text content**. Only insert the annotation tags (`<TAG>...</TAG>`) around identified terms.
- Tags must be complete and properly closed (e.g., `<Drug>atenolol</Drug>`, not `<Drug>atenolol` or `atenolol</Drug>`).
...
Annotated text:  
`Concomitant medications included on an unknown <TEMPO>date</TEMPO>, <Drug>atenolol</Drug> tablet at a dose of <Dose>25 milligrams</Dose> twice a day via unknown route for unknown indication.`

---

### 🔖 Annotation Types and Their Descriptions  
Use the following tags (case-sensitive):

| Tag       | Full Name                               | Definition/Explanation |
|-----------|-----------------------------------------|------------|
| Drug      | Drug Products                           |   |
| AE        | Adverse Event                           |   |
| Treatment | Drug used for Treatment                 |   |
| RO        | Rule-out Diagnosis                      |   |
| Dx        | Diagnostic Test Results                 |   |
| CoD       | Cause of Death                          | Symptom or Adverse Events that specifically cause death |
| Age       | Age                                     | 57 years old, etc.  |
| Sex       | Sex/Gender                              | like Male, Female, etc. |
| Dose      | Dose Administered                       |   |
| bSYM      | Baseline Symptom before AE Occurred     |   |
| Hx        | Medical History                         |   |
| Status    | Patient Status                          |   |
| Lab       | Laboratory Finding                      |   |
| TEMPO     | Temporal Information (e.g., date, time) |   |
